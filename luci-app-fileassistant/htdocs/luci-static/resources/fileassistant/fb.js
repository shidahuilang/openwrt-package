...
(function () {
  ...
  function chmodPath(filename, isdir) {
    ...
    if (newmod) {
      iwxhr.get('/cgi-bin/luci/admin/services/fileassistant/chmod',
        {
          filepath: concatPath(currentPath, filename),
          newmod: newmod
        },
  ...
  function chownPath(filename) {
    ...
    if (newown) {
      iwxhr.get('/cgi-bin/luci/admin/services/fileassistant/chown',
        {
          filepath: concatPath(currentPath, filename),
          newown: newown
        },
  ...
  document.getElementById('mkdir-toggle').onclick = function() {
    ...
    if (dirname = prompt("请输入文件夹名称：")) {
      ...
      var xhr = new XMLHttpRequest();
      xhr.open("POST", "/cgi-bin/luci/admin/services/fileassistant/mkdir", true);
      xhr.onload = function() {
        if (xhr.status == 200) {
          var res = JSON.parse(xhr.responseText);
          refresh_list(res.data, currentPath);
