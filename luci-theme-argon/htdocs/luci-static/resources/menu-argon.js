...
return baseclass.extend({
	...
	render: function(tree) {
		...
		document.querySelector('a.showSide').addEventListener('click', ui.createHandlerFn(this, 'handleSidebarToggle'));
		document.querySelector('.darkMask').addEventListener('click', ui.createHandlerFn(this, 'handleSidebarToggle'));
	},
	handleMenuExpand: function(ev) {
		...
		document.querySelectorAll('.main .main-left .nav > li >ul.active').forEach(function(ul) {
			if(slide_menu){//判断是否为退出按钮
				if (ul == slide_menu) slide_self = true;//判断是否为自身
		...
		if (!slide_menu) return;//退出按钮操作
		if (slide_self) {ev.preventDefault();ev.stopPropagation();return};//为自身则到此为止
