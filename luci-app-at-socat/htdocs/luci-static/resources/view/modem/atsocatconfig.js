...
return view.extend({
	...
	render: function(devs) {
		var m, s, o;
		m = new form.Map('atsocat', _('Configuration socat'), _('Configuration panel for socat.'));

		s = m.section(form.TypedSection, 'atsocat', '', _(''));
		s.anonymous = true;
		...
		o = s.option(form.TextValue, '_tmpl', _('User AT commands'),
			_("Each line must have the following format: 'At command description;AT command'. For user convenience, the file is saved to the location <code>/etc/modem/atsocatcommands.user</code>."));
		o.rows = 20;
		o.cfgvalue = function(section_id) {
			return fs.trimmed('/etc/modem/atsocatcommands.user');
		};
		o.write = function(section_id, formvalue) {
			return fs.write('/etc/modem/atsocatcommands.user', formvalue.trim().replace(/\r\n/g, '\n') + '\n');
		};
