...
function index()
    local function store_api(action, onlypost)
        local e = entry({"admin", "services", "store", action}, onlypost and post("store_action", {action = action}) or call("store_action", {action = action}))
    ...
    entry({"admin", "services", "store"}, call("redirect_index"), _("iStore"), 31)
    ...
    if nixio.fs.access("/usr/lib/lua/luci/view/store/main_dev.htm") then
        entry({"admin", "services", "store", "dev"}, call("store_dev")).leaf = true
    end
    entry({"admin", "services", "store", "token"}, call("store_token"))
    entry({"admin", "services", "store", "log"}, call("store_log"))
    entry({"admin", "services", "store", "uid"}, call("action_user_id"))
    entry({"admin", "services", "store", "upload"}, post("store_upload"))
    entry({"admin", "services", "store", "check_self_upgrade"}, call("check_self_upgrade"))
    entry({"admin", "services", "store", "do_self_upgrade"}, post("do_self_upgrade"))

    entry({"admin", "services", "store", "get_support_backup_features"}, call("get_support_backup_features"))
    entry({"admin", "services", "store", "light_backup"}, post("light_backup"))
    entry({"admin", "services", "store", "get_light_backup_file"}, call("get_light_backup_file"))
    entry({"admin", "services", "store", "local_backup"}, post("local_backup"))
    entry({"admin", "services", "store", "light_restore"}, post("light_restore"))
    entry({"admin", "services", "store", "local_restore"}, post("local_restore"))
    entry({"admin", "services", "store", "get_backup_app_list_file_path"}, call("get_backup_app_list_file_path"))
    entry({"admin", "services", "store", "get_backup_app_list"}, call("get_backup_app_list"))
    entry({"admin", "services", "store", "get_available_backup_file_list"}, call("get_available_backup_file_list"))
    entry({"admin", "services", "store", "set_local_backup_dir_path"}, post("set_local_backup_dir_path"))
    entry({"admin", "services", "store", "get_local_backup_dir_path"}, call("get_local_backup_dir_path"))
...
function store_dev()
    luci.template.render("store/main_dev", {prefix=luci.dispatcher.build_url(unpack({"admin", "services", "store", "dev"})),id=user_id()})
end
